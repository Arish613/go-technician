...
const categories = [
  ...
  { name: "Appliances Repair", icon: "/icons/appliances.svg" },
  { name: "Home Cleaning", icon: "/icons/cleaning.svg" },
  { name: "Laundry", icon: "/icons/laundry.svg" },
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-2 xl:grid-cols-3">
              {categories.map((category) => (
                <Card
                  key={category.name}
                  className="relative cursor-pointer border-slate-200 transition-all hover:border-purple-300 hover:shadow-md"
                >
                  {category.badge && (
                    <Badge
                      className={`absolute -top-2 left-2 ${category.badgeColor} text-xs text-white`}
                    >
                      {category.badge}
                    </Badge>
                  )}
